    ...
    def get(self, location_name):
        ...
        if location:
            results = db.engine.execute(
                "select * from (SELECT name, latitude, longitude,  SQRT(POW(69.1 * (latitude - {latitude}), 2) +"
                "POW(69.1 * ({longitude} - longitude) * COS(latitude / 57.3), 2)) AS distance "
                "FROM locations ) as location where location.distance < 25 ORDER BY location.distance".format(
